      ...
      <Card className="bg-card/60 border-border/50">
        ...
        <CardContent className="p-4 pt-0">
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {fetchError ? (
              <p className="text-xs text-red-400/70 py-4 text-center">Failed to load thought logs</p>
            ) : thoughts.length === 0 ? (
              <p className="text-xs text-muted-foreground/60 py-4 text-center">No thought logs yet</p>
            ) : (
              thoughts.map((t, i) => (
                <div key={i} className="bg-background/50 rounded-lg p-3 border border-border/40">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-[11px] text-muted-foreground">Cycle #{t.cycle}</span>
                    <span className="text-[10px] text-muted-foreground/60">{t.time ? new Date(t.time).toLocaleTimeString('en-IN') : ''}</span>
                  </div>
                  <p className="text-xs text-foreground/80 mb-2">{t.decision?.reasoning}</p>
                  {t.decision?.scenarios_considered?.map((s, j) => (
                    <div key={j} className="text-[10px] text-muted-foreground/60 ml-2">• {s}</div>
                  ))}
                  {t.snapshot && (
